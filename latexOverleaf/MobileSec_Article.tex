...
\begin{figure*}
\centering
\includegraphics[width=0.9\linewidth]{diagram-export-25-12-2025-23_13_53.png}
...
\begin{figure*}
\centering
\includegraphics[width=0.9\linewidth]{sign_up_sign_in.png}
...
\begin{figure*}
\centering
\includegraphics[width=0.9\linewidth]{APK_Scanner.png}
...
\begin{figure*}
\centering
\includegraphics[width=0.9\linewidth]{dashboard_page.png}
...
%----------------------------------------------------------------------------------------
\section{Automatisation du Pipeline DevOps}
\label{sec-devops}

Pour assurer la fiabilité et la reproductibilité du déploiement de \textbf{MobileSec-MS}, nous avons migré d'un processus manuel vers un pipeline \textbf{CI/CD} entièrement automatisé. Cette transition s'appuie sur une stack technologique robuste : Docker pour la conteneurisation, Docker Compose pour l'orchestration, et Jenkins pour l'automatisation.
...
\noindent\fbox{%
    \parbox{0.95\linewidth}{%
        \centering
        \vspace{0.5em}
        \large\textbf{Architecture du Pipeline CI/CD Unifiée}
        \vspace{0.5em}
        \\
        \small
        Le pipeline transforme le code source en un système déployé via 4 étapes majeures automatisées.
        \vspace{1em}
        \\
        \ttfamily
        \begin{tabular}{c c c c c c c}
            \fbox{Git Checkout} & $\rightarrow$ & \fbox{Port Cleanup} & $\rightarrow$ & \fbox{Conteneurisation} & $\rightarrow$ & \fbox{Health Checks} \\
            \scriptsize(Branch: CI-CD-Pipleline) & & \scriptsize(PowerShell Script) & & \scriptsize(Docker Compose Build) & & \scriptsize(Verification)
        \end{tabular}
        \vspace{0.5em}
    }%
}
...
L'implémentation de ce pipeline résout plusieurs défis critiques du développement distribué :

\subsection{Conteneurisation Uniforme}
Chaque microservice (Gateway, APK Scanner, Report Service) possède son propre \texttt{Dockerfile} optimisé.
\begin{itemize}
    \item \textbf{Backend Java :} Utilise des images de base \texttt{eclipse-temurin:21-jre-alpine} pour minimiser la surface d'attaque et la taille de l'image. Les builds sont exécutés via Maven en mode \textit{Batch} (\texttt{-B}) pour réduire le bruit dans les logs.
    \item \textbf{Backend Python :} Utilise des images légères, installant uniquement les dépendances nécessaires au Runtime ML (XGBoost, Pandas).
\end{itemize}

\subsection{Gestion des Conflits de Port}
Un défi majeur dans l'automatisation locale sous Windows est la persistance des ports (ex: 3000, 8080). Notre pipeline intègre un script PowerShell intelligent (\texttt{ci\_cleanup.ps1}) qui s'exécute avant chaque build. Il identifie non seulement les conteneurs Docker obsolètes, mais aussi les processus Windows natifs (PID) qui verrouillent les ports critiques, garantissant un environnement propre ("Clean Slate") à chaque déploiement.

\subsection{Orchestration et Déploiement}
Le fichier \texttt{docker-compose.yml} agit comme la source de vérité pour l'infrastructure. Jenkins invoque la commande \texttt{docker-compose up -d --build}, qui :
\begin{enumerate}
    \item Construit toutes les images en parallèle.
    \item Crée un réseau virtuel interne isolé pour la communication inter-services.
    \item Monte les volumes nécessaires pour la persistance de la base de données PostgreSQL.
\end{enumerate}

Cette approche "Infrastructure as Code" garantit que l'environnement de développement local est strictement identique à l'environnement de production, éliminant le syndrome "ça marche sur ma machine".
